...
public class Math3d {
	...
	public static bool ProjectPointOnLineSegment(out Vector3 projectedPoint ,Vector3 linePoint1, Vector3 linePoint2, Vector3 point){

		Vector3 vector = linePoint2 - linePoint1;

		Vector3 _projectedPoint = ProjectPointOnLine(linePoint1, vector.normalized, point);

		int side = PointOnWhichSideOfLineSegment(linePoint1, linePoint2, _projectedPoint);

		//The projected point is on the line segment
		if(side == 0){
			projectedPoint = _projectedPoint;
			return true;
		}
	...
	public static float MouseDistanceToLine(Vector3 linePoint1, Vector3 linePoint2){
		...
		ProjectPointOnLineSegment(out projectedPoint, screenPos1, screenPos2, mousePosition);
